  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black to-zinc-900 text-white py-8 px-4">
      <div className="max-w-6xl mx-auto">
        {/* Title */}
        <h1 className="text-2xl sm:text-4xl text-center mb-10 bg-gradient-to-tr from-emerald-300 to-cyan-500 bg-clip-text text-transparent animate-fade-in drop-shadow-md tracking-wide">
          Lost & Found Board
        </h1>
          ...
          {["all", "lost", "found"].map((type) => (
            <Button
              ...
              className={`capitalize h-8 px-4 text-sm ${
                filter === type
                  ? "bg-gradient-to-bl from-cyan-600 to-emerald-600 text-white"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredItems.length === 0 ? (
            ...
            filteredItems.map((item) => {
              const isCreator =
                session?.user?.id?.toString() === item.creater?.toString();
                  ...
                  <CardHeader className="pb-2">
                    <div className="flex justify-between items-start text-gray-300">
                      ...
                      <div className="flex flex-col items-end">
                        ...
                        {item.urgent && (
                          <div className="text-red-500 text-xs flex items-center gap-1 font-semibold animate-pulse mt-1">
