...
export default function EventTablePage() {
  ...
  // Upload images for completed event
              ...
              {filteredEvents.map((event) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-center">{formatDate(event.dateofevent)}</td>
                    ...
                    <td className="py-3 px-4 text-center flex justify-center gap-2">
                      ...
                      {/* View button */}
                      <button
                        onClick={() => { setSelectedEvent(event); setUploadFiles(null); }}
                      ...
                      {/* {userRole === 'admin' && !event.iscompleted && ( */}
                        <button
        ...
        <div className="mt-12 max-w-md mx-auto rounded-xl p-6 shadow-lg text-black">
          ...
          <Calendar
            tileContent={({ date }) => {
              ...
              return ev ? (
                <p className="text-xs text-teal-400 truncate" title={ev.name}>
                  ● {ev.name} ({formatDate(ev.dateofevent)})
                </p>
              ) : null;
            }}
          />
        ...
        {selectedEvent && (
          <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-zinc-900 text-white rounded-lg w-full max-w-2xl p-6 relative">
              ...
              <p className="text-sm text-gray-400 mb-2">Date: {formatDate(selectedEvent.dateofevent)}</p>
              ...
              <p className="text-sm text-gray-400 mb-4">
                Status: {selectedEvent.iscompleted
                  ? <span className="text-green-400 font-semibold">Completed</span>
                  : <span className="text-yellow-400 font-semibold">Upcoming</span>}
              </p>
  ...
  <>
    ...
    <div className="mt-4">
      <label htmlFor="image-upload" className="block mb-1 font-semibold">
        Select 2–3 Images
      </label>
      <input
        id="image-upload"
        ...
        accept="image/*"
        onChange={(e) => {
          if (e.target.files) setUploadFiles(e.target.files);
        }}
      ...
      {uploadFiles && uploadFiles.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-2 max-h-40 overflow-auto">
          {Array.from(uploadFiles).map((file, index) => (
            <div
              key={index}
              className="relative w-20 h-20 border border-gray-600 rounded overflow-hidden"
            >
              <img
      ...
      <Button
        onClick={async () => {
          if (!uploadFiles || uploadFiles.length === 0) return alert('Select images first');
          ...
          try {
            ...
            setUploadFiles(null);
            fetchEvents(); // Refresh events to show uploaded images
          } catch (err) {
            console.error(err);
